...
驗證成功後，TMAG 會沿用 CallMesh 的授權資訊，將 Meshtastic 之摘要資料同步至 TenManMap（以下稱「本服務」）。若你同意分享，本服務可能將部分資料提供予合作夥伴（例如共同營運、託管與分析服務供應商），以支援服務運作。

- **同步範圍**  
  與節點狀態與現地觀測相關之摘要資訊，例如：位置（含時間戳）、遙測摘要、訊息摘要欄位、以及必要的連線/傳輸品質指標（如 RSSI/SNR、hop、節點狀態）。

- **使用目的**  
  用於以下目的之必要範圍：
  - 服務提供與維運（含故障排除、通知、容量規劃）
  - 效能與可靠度監測（含品質統計、異常偵測）
  - 資安維護（含濫用偵測、稽核紀錄）
  - 統計分析與研究（以彙總或去識別化方式為原則）
  - 依營運所需之功能開發與產品改善  
  （若需將資料用於超出上述目的之新用途，將另行告知並取得必要同意）

- **分享對象**  
  - **TenManMap**：用於本服務之提供與維運。
  - **合作夥伴**：限於協助本服務運作所必要之第三方（例如：雲端託管、資料儲存、訊息推播、監控分析、共同營運單位）。
  - **依法令要求**：於法律要求或有權機關合法程序下，可能提供予主管機關或其他有權單位。

- **保留期間**  
  為達成上述目的所必要之期間，或依法律/合約要求之期間保存。到期後將刪除、匿名化或以其他方式降低識別風險（依實際作業與法規要求）。

- **你的選擇**  
  你可選擇是否同步/分享：
  - 以環境變數停用：`TENMAN_DISABLE=1`（支援 `true`/`yes`/`on`）
  - CLI 參數停用：`--no-share-with-tenmanmap`
  - 桌面版：設定頁底部之分享開關  
  停用後，本服務將不再接收新的同步資料；已產生之資料仍可能因備援、稽核或法規要求而於合理期間內保留。

- **資料安全**  
  我們採取合理之技術與管理措施（例如存取控管、傳輸保護、稽核與最小權限）以降低風險，但仍無法保證在所有情境下絕對安全。

- **跨區處理**  
  資料可能在不同法域之間處理與儲存。我們將依適用法規與合理措施管理跨境處理風險。

- **權利行使與聯絡方式**  
  如需查詢、請求刪除、或停止同步/處理，請依你的部署流程聯絡維運窗口或系統管理者；若你是自行部署者，請由部署者提供對應的聯絡方式或工單管道。

- **條款更新**  
  本段內容可能依實際作業調整。我們會在版本更新說明、文件頁面或公告渠道揭露更新內容與生效日期。
